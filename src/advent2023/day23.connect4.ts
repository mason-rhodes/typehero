...
type GetRowIndex<
  ...
  Store extends unknown[] = []
> = Column extends [infer First, ...infer Rest]
...
> = true extends
  ...
  | CheckDiagonalForWin<
      Board,
      Chip,
      DiagonalMap[GetRowIndex<GetColumn<Board, Column>>][Column]
    >
  ? Chip extends "🔴"
    ? "🔴 Won"
    : "🟡 Won"
  : Connect4EmptyCell extends Board[0][number]
  ? Chip extends "🔴"
    ? "🟡"
    : "🔴"
  : "Draw";
...
> = Game["state"] extends Connect4Chips
  ? {
      ...
      state: CheckForWin<
        MakeMove<Game["board"], Game["state"], Move>,
        Game["state"],
        Move
      >;
    }
